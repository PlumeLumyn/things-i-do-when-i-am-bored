...
static void my_sigint_handler(int sig, siginfo_t *info, void *ctx) {
  ...
  if (gscreen_prev_sigint.sa_flags & SA_SIGINFO) {
    if (gscreen_prev_sigint.sa_sigaction &&
        gscreen_prev_sigint.sa_sigaction !=
...
public:
  ...
  int getSubPixelHeight() { return h * 2; }
  int getX() { return x; }
  int getY() { return y; }
  ...
  void putPixel(int32_t x, int32_t y, float z, Vec3f penCol) {
    if (x < this->getWidth() && y < this->getSubPixelHeight() && x >= 0 &&
        y >= 0) {
  ...
  void refresh() {
    ...
    for (size_t i = 0; i < this->h; i++) {
      ...
      for (size_t j = 0; j < this->w; j++) {
        ...
        if (character == "▄") {
          std::cout << "\033[48;2;" << fgCol[0] << ";" << fgCol[1] << ";"
                    << fgCol[2] << "m";
          std::cout << "\033[38;2;" << bgCol[0] << ";" << bgCol[1] << ";"
                    << bgCol[2] << "m";
        } else {
          std::cout << "\033[38;2;" << fgCol[0] << ";" << fgCol[1] << ";"
                    << fgCol[2] << "m";
          std::cout << "\033[48;2;" << bgCol[0] << ";" << bgCol[1] << ";"
                    << bgCol[2] << "m";
        }
        std::cout << character;
      }
...
public:
  ...
  Window &getWindow(size_t id) { return windows[id]; }
