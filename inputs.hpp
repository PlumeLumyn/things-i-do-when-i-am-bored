...
static void my_sigint_handler(int sig, siginfo_t *info, void *ctx) {
  ...
  if (ginputs_prev_sigint.sa_flags & SA_SIGINFO) {
    if (ginputs_prev_sigint.sa_sigaction &&
        ginputs_prev_sigint.sa_sigaction !=
...
private:
  void loop() override {
    char buf[64];
    if (isPressed(Keycode::LeftControl) && isPressed(Keycode::C))
      raise(SIGINT);
    ...
    if (bytesRead > 0) {
      std::string symbol(buf, bytesRead);
      size_t index;
      if ((index = symbol.find(0x1b)) == SIZE_MAX) {
        return;
      }
      symbol = std::string(buf + index + 1, bytesRead - index - 1);
      // std::cout << symbol << std::endl;
      size_t rawKeycode;
      sscanf(symbol.c_str(), "[%ld;", &rawKeycode);
      Keycode keycode = toKeycode(rawKeycode);

      index = symbol.find(";");
      if ((index = symbol.find(";")) == SIZE_MAX) {
        return;
      }
      symbol = std::string(buf + index + 5, bytesRead - index - 5);
      if (symbol[0] == 'u') {
