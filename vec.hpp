...
template <typename T, size_t W, size_t H> constexpr Mat<T, W, H> identity() {
  Mat<T, W, H> res;
...
T _vectorsDotProd(const Vec<T, H> &x, const Vec<T, H> &y, size_t n) {
  T res = 0.0;
  int i;
  for (i = 0; i < n; i++) {
    res += x[i] * y[i];
...
void MatrixVectorMult(
    const Mat<T, W, H> &mat, const Vec<T, H> &vec,
    Vec<T, H> &result) { // in matrix form: result = mat * vec;
  int i;
  for (i = 0; i < H; i++) {
    result[i] = _vectorsDotProd<T, H>(mat[i], vec, W);
  }
}
...
template <typename T, size_t W, size_t H>
T _multiplyRowByColumn(const Mat<T, W, H> &mat1, const int row,
                       const Mat<T, W, H> &mat2, const int col) {
  T sum = 0;
...
template <typename T, size_t W, size_t H>
void _matmul(const int start, const int end, const Mat<T, W, H> &A,
             const Mat<T, W, H> &B, Mat<T, W, H> &C) {
...
template <typename T, size_t H> inline Vec<T, H> Negative(Vec<T, H> vec) {
  for (int i = 0; i < H; i++)
    vec[i] = -vec[i];
  return vec;
}
...
inline Mat4f BuildViewMatrix(Vec3f &cp, Vec3f &ca) {
  ...
  Mat3f x = {{{1, 0, 0}, {0, cosx, sinx}, {0, -sinx, cosx}}};
  Mat3f y = {{{cosy, 0, -siny}, {0, 1, 0}, {siny, 0, cosy}}};
  Mat3f z = {{{cosz, sinz, 0}, {-sinz, cosz, 0}, {0, 0, 1}}};
...
inline Mat4f BuildProjectionMatrix(float fovRad, float aspect, float near,
                                   float far) {
  float tanHalf = tan(fovRad / 2.0f);
  float f_n = far - near;

  return {{{1.0f / (aspect * tanHalf), 0, 0, 0},
           {0, 1.0f / tanHalf, 0, 0},
...
template <typename T, size_t N> Vec<T, N> &operator*=(Vec<T, N> &v, T scalar) {
  for (size_t i = 0; i < N; ++i)
    v[i] *= scalar;
  return v;
}
